...
| Отправить запрос на добавление тарифа со всеми обязательными ключами json, в заголовках передать autharization bearer token | HTTP 403. В теле ответа содержится сообщение о запрете для данного пользователя. |
...
| step | expected result |
|---|---|
| Отправить запрос на добавление тарифа со всеми обязательными ключами json, в заголовках не передавать autharization bearer token | HTTP 401. В теле ответа содержится сообщение о необходимости токена авторизации. |
...
|---|---|
| Авторизоваться администратором. | HTTP 200. В теле ответа содержится поле token, значение которого можно передавать в заголовки защищённых запросов. |
| Отправить запрос на создание тарифа с валидными данными и autharization bearer token в заголовках 2 раза. | При первой отправке – HTTP 2xx, при второй – HTTP 4xx и в теле ответа сообщение о том, что тип уже используется. |
...
|---|---|
| Авторизоваться администратором. | HTTP 200. В теле ответа содержится поле token, значение которого можно передавать в заголовки защищённых запросов. |
| Отправить запрос на просмотр списка тарифов и выбрать случайный идентификатор. | Идентификатор выбран. |
| Отправить запрос на удаление тарифа с выбранным id в query параметрах, в заголовках передать autharization bearer token. | http 2xx. |
| Отправить повторный запрос на удаление тарифа с id, который использовался ранее для удаления, в заголовках передать autharization bearer token. | HTTP 4xx. В теле сообщение о том, что тариф не найден. |
...
| Отправить запрос на просмотр списка тарифов и выбрать случайный идентификатор. | Идентификатор выбран. |
| Отправить запрос на удаление тарифа с выбранным id в query параметрах, в заголовках передать autharization bearer token. | http 403. |
...
|---|---|
| Отправить запрос на просмотр списка тарифов и выбрать случайный идентификатор. | Идентификатор выбран. |
| Отправить запрос на удаление тарифа с выбранным id в query параметрах. | http 401. В теле ответа содержится сообщение о необходимости токена авторизации. |
...
|---|---|
| Авторизоваться под ролью ADMIN. | HTTP 200. В теле ответа содержится поле token, значение которого можно передавать в заголовки защищённых запросов. |
| Отправить запрос на удаление тарифа с id=-1 в query параметрах, в заголовках передать autharization bearer token. | HTTP 4xx. Возвращается корректное сообщение об отсутствии тарифа. |
---